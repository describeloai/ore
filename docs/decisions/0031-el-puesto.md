...
| **W3.7** ✓ 2026-09-22 · declarar, correr y **el gobierno de lo escrito** | **hecho**: ①–④ (la firma y la rama, el modelo entrenado, el linaje, el trabajo) y el gobierno en seis pasos (abajo, «Lo construido para W3.7 gobierno»): desde un puesto sólo entran los verbos; la lectura desde código es un conducto; la clasificación baja por `derivedFrom` hasta lo escrito; lo escrito es de quien lo escribió y el puesto nace en su rama; lo declarado lo conoce el servidor; y la cuenta del puesto deja de ver el bucket. *(Antes decía:)* `transform(inputs, output)` en los tres; el trabajo de código desde un commit (`ore run packages/p/transforms/x.{py,ts,java}`) con entorno + capa; fallback de rama; un `over` no declarado se rechaza | medido: frío del trabajo por entorno, un transform sobre 200 M de filas |
| **W3.8** · baja latencia y **la promoción** | funciones TS y Python residentes (0029 ②): un proceso por función con sus vistas calientes, invocado por `ore-serve`. **Anotado el 2026-09-21, tras W3.7 ②**: los intérpretes ya están —las tres imágenes del puesto ejecutan Python (`exec`), TS (el REPL de Node) y Java (JShell en proceso)—; lo que no está es **correrlos como una `Function`**: `runtime: python\|node\|jvm` con `entrypoint: transforms/x.py`, `ore-invoke` lanzándolo con la imagen del puesto (bajo demanda: el Job de «Lo medido para W3.7» §5, 48 s de frío con 31 de testigo; residente: un proceso vivo por función). Es «guardar como función» (Foundry): la **promoción** del código de nivel 2 (un fichero del repo, registrado por sus frutos) a nivel 3 (un documento con contrato y sandbox), explícita y no automática, y el decorador `transform(inputs, output)` de ③ es su material (`reads`/`output`). Toca 0029 en dos frases y hay que decirlo ahí, no rodearlo: (1) el aislamiento pasa de «red cerrada **más** WASI sin sockets» a **sólo la red cerrada** (un `def` de Python abre sockets; que no lleguen a nada lo garantiza la `NetworkPolicy`, que es la garantía con la que ya corren los puestos y los Jobs de código); (2) «`runtime: wasm` es a propósito la única forma de traer código» sigue valiendo contra el contenedor del cliente, y una imagen **nuestra** que corre código del cliente no lo es. Y un hueco que la medida destapó: el compilador **no aplica** el enum `runtime: wasm\|model` del schema (`runtime: python` compila hoy); se cierra en la dirección que se decida | medido: p50/p99, memoria de las vistas calientes, arranque; y el frío por invocación frente a lo que una `Action` desde una pantalla tolera |
...
Leer el lago como origen de `materialize` ya no pide driver: una View cuya raíz es una
`Table` del lago se copia en Arrow por `ore-store copiar` (abajo, «(d)»); `ask` sigue por la
copia.
...
que es lo que la tabla tiene que valer para que nadie tenga que fundir al leer.

**(d, 2026-09-21) materializar sobre el lago, sin driver.** Lo medido en §5 decía que el
protocolo de texto de 0008 es el camino lento para lago → lago (8–14 s/1 M frente a 0,4 s en
Arrow) y que un driver ni siquiera ve el puntero. Así que no hay `ore-read-lago`: cuando
`lector::declaracion` de la raíz de la vista dice `type: lago`, `materialize` toma el testigo
del puntero que el árbol ya tiene (`datasets/<objeto>.json`: `witness: snapshot` → el
`snapshot` del puntero; `none` → nada; `log` o `field` sobre una tabla del lago se niegan con
el nombre), y en vez de `ore-read-<tipo> leer` + `ore-store sellar` llama a **`ore-store copiar`**:
la misma cabecera que `sellar` más `origen: {metadata_location, dataset, proyeccion, filtros}`
—la petición que un driver recibiría, sin la URL—. El almacén abre la tabla origen por su
puntero, lee sus lotes vivos (`Lago::lotes`, con los position deletes aplicados), aplica los
filtros `eq` con el literal llevado al tipo de la columna (`arrow_cast` + `arrow_ord::eq`: un
`2.5` contra un decimal es `2.50`), proyecta por nombre y lleva cada columna al físico que la
cabecera declara (`carga::arrow_del_oos`, el mismo `DataType` que `carga::lote` daría al texto;
lo que no convierte se queda texto y va a `sin_estrechar`), y de ahí es la cola de `sellar`
(`confirmar_copia`, compartida): la cabecera en el snapshot, `creada`/`sobrescrita`, las cuentas
por columna, y `leidas` —las filas del origen, la unidad de 0014— que el puntero guarda. Un
snapshot no ordena, así que sobre el lago se copia entera y es la cabecera igual (el mismo
snapshot) la que corta el ciclo en ④ sin leer nada. El orden de las filas es el del origen (no
se ordena por clave como con el texto). Pruebas: `copiar_proyecta_filtra_y_sella_sin_texto`
(ore-store), `el-lago.sh` 14 (`ventas.francia` sobre `ventas.py` —la tabla que DuckDB mutó, 267
filas vivas— da 1 fila de 267 leídas con `where: { pais: FR }`; `ventas.todoPy` 267 sin las
borradas; el testigo es el snapshot del puntero de `ventas.py`; `ore ask` las contesta tipadas;
la segunda pasada «ya está»; una `Table` del lago que nadie escribió se dice con la ruta de su
puntero). Lo que sigue fuera: `ore-read-lago` como driver de texto (no hace falta), y que
`copiar` sirva un rango (sin cursor no hay incremento que pedir a un snapshot).

El nodo de `jobs-p` no baja solo mientras haya un puesto abierto o un
...
## Lo medido para W3.7 · declarar y correr (`pruebas-de-fuego/medida-w3-declarar.py`, 2026-09-21; en local con una forja pelada, el S3 de mentira y `agente.py` de verdad; §5 en `t-demo`)

Lo que **declarar** tiene que ser: el puente entre lo que una sesión de código produce —un
dataset, una vista sobre él, una entidad, una interfaz, una función, un modelo— y el registro
de assets del inquilino, que es el árbol (el Assets Catalog lo lee). Antes de escribirlo, qué
parte del puente ya está y qué no:

| | medido | lo que se sigue |
|---|---|---|
| **§1 el puente hoy, desde una celda** | `write()` → dataset: 3,6 s, el puntero y la `Table` **firmados por `persona:ana`** (`x-ore-puesto` → `persona_del_puesto`), la ficha con `escrito_por`. `PUT /documentos/{kind}` desde la misma celda (con el testigo del agente): **View, Concept, Entity e Interface entran (201, 1,0–1,6 s: clonar + compilar + empujar) pero el commit lo firma `agente:local`**: la puerta de Forge usa el sujeto crudo, no honra `x-ore-puesto` ni la rama del puesto. La misma View con el testigo de la persona: 201 firmado por ella. `Function` y `Model`: **404** («no se sirve por `/documentos`»). `ore validate` **acepta `runtime: python`** en una Function (el enum `wasm\|model` del schema no lo aplica el compilador) y acepta el `Model` de v1alpha9 (`profile/tier/task`: el LLM servido), pero un `Model` con pesos (0031 §6: `base`, `version`, `artefactos`) es `OOS1005` | La puerta ya existe para cuatro kinds; lo que falta es **una firma y una rama**: que `/documentos` resuelva el puesto como lo hace `/v1` y `/datasets`. `Function` y `Model` son dos filas más de `KINDS` (la de Function con sus exigencias); el `Model` de pesos no existe en OOS |
| **§2 la rama** | Un puesto abierto con `rama: bea/w37` escribe **en la rama** (`datasets/ventas_enRama.json` en la rama, no en `main`; `GET /arbol/historia` de `main` 404). `over()` desde la rama de lo que **ya estaba** en `main` al abrirla: 3 filas; de lo que `main` ganó **después**: «no hay ninguna `View`» (**no hay fallback**: se lee el árbol de la rama tal cual, y el mensaje habla de una View que no es el caso). Desde `main`, lo de la rama: no está. `POST /ramas` y `/ramas/{r}/fusionar`: 422 sin API de forja (en la celda de verdad la hay) | La rama ya aísla lo que una sesión escribe. Publicar = **fusionar la rama en `main`** (la ruta existe con forja), no otro commit. El fallback de §4 («una rama lee las copias de `main` mientras no tenga las suyas») **no está hecho**, y el error lo dice mal |
| **§3 el linaje** | El puntero de un dataset escrito: 9 claves (`dataset, escrito_por, estado, filas, metadata_location, operacion, snapshot, tabla, uuid`); la ficha **no dice de qué salió ni con qué conducto**; la tabla Iceberg lleva 3 propiedades (retención, `ore.dataset`), ninguna de procedencia. `ore view` de una View encima: linaje «raíz lago · ventas_salida», y ahí se acaba. Lo que una `Function` de OOS ya exige: `reads` **sólo Views** (`OOS7014` sobre una Table), `effects.writes` **una propiedad de una Entity** (`<ns>.<Entidad>.<prop>`, `OOS2005`) cuya vista de respaldo esté **materializada** (`OOS2025` si es virtual) | Un dataset escrito hoy es una **hoja sin padres**: el linaje que 0031 §10 promete (`salida ← transform ← inputs`) sólo puede escribirlo quien lo produjo, y eso es el verbo. El contrato de `Function` (leer vistas, escribir propiedades por la copia) es el de una acción sobre la ontología, **no el de un transform** que produce un dataset nuevo: `transform(inputs, output)` es otra cosa y necesita su forma |
| **§4 correr, en local** (1 M de filas, `ore-store` de release) | La tabla en Arrow en la celda 0,4 s; `write()` de 1 M **3,4 s** (IPC → `ore-store escribir` → commit); `sql()` que agrega sobre ella 1,1 s; `write()` del resumen (4 filas) 3,0 s —**el suelo de una escritura son ~2,5–3 s de clonar y empujar**, no las filas—; `write(over())` de 1 M 4,6 s; el agente con la tabla viva 214 MB de RSS; `over()` de la copia 0,6 s | Un transform de 1 M cabe en la sesión con segundos de margen; a 200 M el límite es la memoria del pod (3 GB: ~40 M de filas de 3 columnas en Arrow), no el tiempo. **El SDK materializa la tabla entera** antes de escribir: para lo que no quepa hace falta escribir por lotes (`write()` de un `RecordBatchReader`) |
| **§5 correr, en `t-demo`** (`puesto-python:5f3e17a`, con el nodo ya arriba por los puestos de victor) | Job creado 1,4 s; pod programado en 1 s; **el testigo (`gcloud secrets` en el init de `ore-drivers`) 31 s**; la imagen del puesto 1 s (en caché del nodo); el guion 15 s: python 0,3 s, testigo 0,4 s, tabla 1,1 s, `write()` de 1 M **6,2 s** (a GCS con el token prestado), `sql()` que agrega 2,6 s (vuelve a leer del bucket), `write()` del resumen 4,6 s. Total 48 s, de los que 31 son el testigo. Sin nodo: +56–110 s de `jobs-p` 0 → 1 (medido en «Lo medido», frío). Todo retirado (documentos y punteros por `DELETE /arbol/…`, objetos del bucket) | **Un trabajo de código ya corre hoy** con la imagen del puesto, el SA `puesto` y el testigo del agente: lo que `ore run` tiene que añadir es el guion desde el commit y el informe al árbol, no otra infraestructura. El coste fijo que se ve es el testigo por `gcloud` (31 s): el agente lo saca en 0,4 s por HTTP, y el init debería hacer lo mismo |
...
| **②** el modelo entrenado | **Decidido antes de escribir** (2026-09-21): un transform **no es un documento de OOS** —es código en un commit más el dataset que produce, que ya tiene `Table` y puntero; su procedencia va al puntero (③)—, y la `Function` de 0029 queda como lo que es (una acción sobre la ontología, en wasm). Lo que sí faltaba es el modelo con pesos, y **no es el `Model` de v1alpha9** (un perfil servido, sin namespace, en la raíz, que dice de sí «no lleva pesos»): es **`TrainedModel`, OOS v1alpha11 · publicar** (`C:\oos` `ec16a70`): un documento del paquete que lo entrenó, con `owner`, `framework` (texto), `task` (opcional), `version` (≥ 1), `artifacts` (prefijo del lago), `digest` (del manifiesto) y `trainedFrom` (vistas, un conjunto: por ahí baja la gobernanza); sin `labels` ni métricas. Ningún código nuevo (`OOS1004/1005/2005/2009`); que los ficheros estén y el digest sea el suyo lo comprueba quien publica (④). ORE: `ApiVersion::V1Alpha11`, `Kind::TrainedModel` (forma, `trainedFrom` → `OOS2005`, `owner` → `OOS2009`, del paquete, `trainedFrom` conjunto en la forma canónica), fila en `KINDS` (`packages/<ns>/models/`); conformance 6/6 | **hecho** 2026-09-21: `el-puesto.sh` 10 (`declare(TrainedModel)` con linaje por la View declarada: en el árbol y listado por `GET /documentos/TrainedModel`; linaje roto `OOS2005`; sin digest `OOS1004`; nada de lo negado queda) |
...
| paso | qué | estado |
|---|---|---|
| **①** la puerta del puesto | regla 1: `/arbol`, `/ramas`, `/trabajos`, `/propuestas` niegan al agente con `x-ore-puesto` (403 «desde un puesto sólo entran los verbos»); `el-puesto.sh` lo prueba desde la celda. `DELETE /documentos/Dataset` retira el puntero. La medida §2 pasa de 200/200/201 a 403/403/403 | **hecho** 2026-09-22. `rutas.rs::puerta_del_agente`, antes del `match` y en lista de permitidos (P4, como `mando.rs`): **lo que decide es el sujeto, no la cabecera** —quitar `x-ore-puesto` desde la celda dejaba el mismo testigo del agente—, así que un agente no escribe (nada que no sea `GET`) fuera de `/puestos/…`, `/v1/…`, `/documentos/…`, `/conceptos/…` y `/datasets/{ns}/{n}/confirmar`; leer sigue abierto (las medidas y el índice leen con el agente). `DELETE /documentos/Dataset` retira el puntero en el mismo commit (`puntero: true` en la respuesta). `el-puesto.sh` 12: desde la celda, `PUT`/`DELETE /arbol`, `POST /ramas`, `/propuestas` y `/paquetes` son 403 con y sin cabecera, `GET /arbol` 200, una persona escribe en `/arbol`, y el Dataset se va con su puntero. La medida §2: `PUT /arbol` 200/200 → **403/403**, «puntero: sí» → **no**; §4: `PUT /arbol` desde el transform 201 → **403** |
| **②** el conducto de la lectura | regla 2: `contextSurface.workspace` nace con `autorizar_conducto`; `datos_del_puesto` calcula la clasificación efectiva (compartida con `assets::clasificacion_de`, que se mueve a ore-core) y niega con `OOS4002`; el SDK lo dice en la celda con la etiqueta. La medida §1 pasa de «la columna high, entera» a 403 | **hecho** 2026-09-22, en ore-serve; **②b** (abajo) queda con nombre. `flow::carga_de` (lo que cada campo lleva: el datasource raíz, **las columnas de la `Table` raíz** —vía nueva, 0032 las etiqueta ahí— y las entidades de la cadena) y `flow::fugas` salen de `vistas_materializadas` y los usan los tres: el compilador (`materialization.payload`), el índice (`acceso.clasificacion` de View y Dataset es ahora su carga entera) y **`flow::lectura_desde_puesto`**, que `datos_del_puesto` llama antes de resolver: 403 con `codigo` `OOS4002`/`OOS4001`/`OOS4011` y la frase; `datos` trae `clasificacion`. Lo que salió: **sin `contextSurface.workspace` se coteja con `materialization.payload`** —la copia ya salió por él al bucket del inquilino; leerla desde un puesto del mismo inquilino no la lleva más lejos—, así que demo y victor no cambian hasta que declaren un retículo; `autorizar_conducto` hace nacer los dos. Los tres SDK dicen la frase en la celda (`PermissionError` / `Error` / `SecurityException`). `el-puesto.sh` 13; la medida §1: «la columna high, entera» → **`OOS4002: ventas.salida.total lleva gdpr.sensitivity:high y materialization.payload solo admite low`** en ana, bob, la View y `sql()`; `GET datos` 403 con `codigo` |
| **②b** la credencial de lectura | lo que ① dejó a la vista: en el clúster `over()` lee el bucket **con el token del pod** (`puesto` es `objectViewer` del bucket entero), así que ② sólo gobierna a quien pasa por `datos`. Que la lectura vaya como la escritura: `datos` presta una credencial CAB `objectViewer` acotada a `ore/v2/datasets/<p>_<t>/` (`ore-store prestar`, lo que `/v1` ya hace al escribir) y la cuenta del puesto **deja de ver el bucket** (aprovisionador). Se mide antes en `t-demo` (`jobs-p` 0 → 1 → 0, con go): que el SDK lee con la prestada y que sin `objectViewer` el token del pod no lee nada | **hecho** 2026-09-22 (demo; victor con go). **Medido antes** (`medida-w3-credencial.py`, un Job con la identidad del puesto; victor para leer, demo para la cuenta): el token del pod leía el `metadata.json` de dos datasets y listaba `ore/v2/datasets/` (200, 200, 200); una CAB `objectViewer` sobre la tabla —**STS 50–60 ms**— lee la suya (200), la otra no (403), no lista nada (403), y **DuckDB `iceberg_scan` con ella como bearer lee 4 filas en 558 ms**; con la **condición IAM** puesta en `ore-puesto-demo` (`resource.name.startsWith(…/objects/ore/puesto/)`, sin el `objectViewer` liso) el pod lee un fichero de la capa **por su nombre** (200) y **no lista ni lee nada más** (403 ×4): **una condición sobre el nombre no da `objects.list`**, ni con prefijo. **Lo construido**: `ore-store prestar {modo: leer}` (`prestar_lectura`: sólo `objectViewer`), `ore datasets --cargar --prestar --leer` (sin exigir `escrito_por`: lo de leer lo decide el conducto antes), `Servidor::con_credencial`: `GET /puestos/{id}/datos/{x}` trae `dataset` y **`credencial`** (la de leer, acotada) y los tres SDK leen con ella —un secreto de DuckDB **por raíz, con `scope`** (un `sql()` que junta dos datasets lleva dos)— o con la del pod si no viene (lo de antes); `51-el-puesto.yaml`: **la capa se baja por su nombre** (`requirements.lock` dice qué ruedas hay), sin listar; `aprovisionar-inquilino.sh`: `ore-puesto-<n>` pasa al `objectViewer` con condición, y se le quita el liso si lo tenía; `malla/empujar-plantilla.py`: la plantilla a la cola de un inquilino sin reaprovisionar (demo `f8cc3ac`, victor `6b1536c`). Comprobado en demo tras la condición: el init de la plantilla baja 3 objetos por su nombre en 1,6 s e instala 182 MB. `el-puesto.sh` 13: `datos` trae `credencial` (el S3 de mentira presta lo que tiene). **Lo que queda con go**: la condición IAM en victor —sus puestos abiertos corren el SDK de antes (leen con el token del pod) y se romperían hasta reabrirse con la imagen nueva—; y los sobres heredados (`clave`, `ore/v1/`) dejan de leerse desde un puesto: victor tiene 0 y demo 0 |
| **③** la clasificación por el grafo | regla 3: OOS 01-dataset §5 (una frase, `C:\oos`, bump del submódulo); `ore validate` y el índice bajan la clasificación por `procedencia` (`inputs` o `leidas` sin el propio nombre); `leidas` se cierra al transform cuando lo hay. La medida §5 pasa de «compila» a `OOS4002` y el índice de `{}` a `high` | **hecho** 2026-09-22. **Lo que baja está en el documento, no en el puntero**: el Dataset escrito gana **`derivedFrom`** (OOS v1alpha12 `5d54854`, `vendor/oos`: §3.2, §5 «lo escrito lleva lo que leyó», §7; esquema; conformance `what-code-wrote-says-what-it-read`, `what-code-wrote-carries-what-it-read` → OOS4002, `a-copy-that-says-derived-from` → OOS1004). El compilador no lee estado, y la carga de un dataset tiene que verse con el árbol solo. Lo escribe `ore datasets --commit` en el mismo acto que `columns`, de la procedencia del `add-snapshot`: los `inputs` del transform o las `leidas` de la sesión —**sin él mismo** (los tres SDK) y sólo lo que el árbol tiene—; sobrescribir dice de nuevo, anexar y upsert suman (`Leyo`, `con_clave`). `flow::carga_de` vía 3: si el suelo de la cadena es un escrito con `derivedFrom`, **cada** campo lleva el join de lo que llevan todos los campos de lo leído (recursivo, con guarda) —el código no declara qué columna salió de cuál: P4—; el índice saca `sale_de` del documento (y del puntero si el documento no lo dice: lo escrito antes de hoy). `el-puesto.sh` 14 (y 8/9: lo que Node y Java escriben lo dice). La medida §5: «compila» → **`OOS4002`**, `{}` → **`high`**, la cadena `derivado ← salida` sin ciclo, `derivedFrom: [ventas.salida]` en el documento. Lo que no baja: una `Function` con `reads` sobre la vista derivada sigue compilando —`reads` no pasa por `flow` (es el conducto `datasource` de 0029, otra costura)— |
| **④** quién reescribe, y la rama | regla 4: 409 con `escrito_por` a quien no escribió (sobrescribir, anexar, upsert, retirar); un puesto sin rama nace en `<persona>/puesto`; la consola sabe publicar desde el puesto (propuesta). La medida §2/§3 pasa de 200 a 409 y de `main` a la rama | **hecho** 2026-09-22. **Es 403, no 409**: no es un conflicto que se resuelva reintentando, es que no es suyo (`ForbiddenException` en el catálogo, código 77 de `ore`). `datasets.rs::de_quien_lo_escribio` lo decide en dos sitios —**la credencial** (`--cargar --prestar` lleva ahora el sujeto: a quien no escribió no se le presta) y **el commit**— y `retirar_documento` en el tercero (un Dataset con puntero de otra persona). Sin sujeto (un `ore` a mano) o sin `escrito_por` (una copia, lo de antes de W3.6c) no hay a quién negárselo. **La rama**: `Servidor::rama_del_puesto` —sin rama dicha y con forja, `<prefijo>/puesto`, que `git::Forja::asegurar_rama` crea de `main` por `push HEAD:refs/heads/…` si `ls-remote` no la ve (sin API de forja; en local también); el trabajo igual; sobre un directorio (el banco, las pruebas) no hay ramas—. Lo que la rama no tiene se lee de `main` (③ de W3.7). **Publicar**: rubix-platform `a8e406c` (local): la fila de la sesión enseña la rama y gana «Publish» → `POST /propuestas` desde ella. Pruebas: `el-puesto.sh` 15 (a bea no se le presta la credencial de `hr.derivadoT` —403 con `persona:ana`, `ForbiddenException`—, su commit es 403, no lo retira; ana sí lo retira entero), `git.rs` (la rama nace de `main` una vez). La medida: ana abre sin rama → `ana/puesto`; con lo suyo publicado en `main` (por git, lo que una propuesta aceptada haría), bob abre después y **anexar y sobrescribir `ventas.salida` son 403** con `persona:ana`; lo que bob redeclara (la Entity `total: low`) queda **en `bob/puesto`** y `main` sigue `high`, firmado por ana; bob como persona en `main` retira `ventas.deAna`: 403. Lo que salió: una rama nace de `main` al abrir y **no sigue a `main`** después (bob no ve lo que ana publicó tras abrir él, salvo los datos por el fallback): es git, y se dice; la sesión que quiera lo nuevo publica y vuelve a abrir |
| **⑤** lo declarado, en el servidor | regla 5: `POST/DELETE /puestos/{id}/transform`; `datos_del_puesto` y el catálogo lo aplican; el trabajo lo lleva en el informe. La medida §4 pasa de 200 a 403 a pelo | **hecho** 2026-09-22. `POST /puestos/{id}/transform {nombre, inputs, output}` y `DELETE …/transform` (del agente del puesto, como `pendiente`; dos a la vez → 409): el SDK los llama al entrar y al salir del decorador en los tres lenguajes, y si el servidor no contesta (uno viejo) el SDK sigue acotando por su cuenta. Mientras el transform está, `datos_del_puesto` **sólo resuelve `inputs`** y el catálogo **sólo carga, esboza o confirma el `output`** (`tabla_de`: de la ruta, o del cuerpo en un `createTable`; un `commitTransaction` lo miran las tablas que trae, que ya pasan por `--commit`). `GET /puestos/{id}` dice `transform`/`inputs`/`output` mientras corre, y el informe del trabajo gana `declarado`. `el-puesto.sh` 16 (a pelo, rodeando el SDK: `datos` de lo no declarado 403, el catálogo de otra tabla 403, `GET /puestos` lo dice; al salir se retira y todo vuelve a resolverse). La medida §4: `[PermissionError, 200, 200, …]` → **`['PermissionError', 403, 403, 'resumir', ['ventas.salida'], 3]`**. Lo que queda dicho: la procedencia **sigue siendo lo que el SDK vio** —quien rodea el SDK ya no lee nada de fuera, pero lo que lea de sus `inputs` sin decirlo no aparece—; y un `commitTransaction` de varias tablas no pasa por la puerta del `output` (las mira `--commit`, que es quien puede) |
| **⑥** medido de nuevo, y 0031 | `medida-w3-gobierno.py` otra vez; los números en esta ADR; el brief se va | pendiente |

## Lo construido para W3.7 gobierno (2026-09-22): el gobierno de lo escrito, de la medida a la celda

Con un brief desechable (`docs/gobierno.md`, borrado al cerrar esto) y en el orden del método:
la medida entera antes de decidir, la decisión antes del código, y la misma medida al final.

| paso | qué | lo que quedó |
|---|---|---|
| **medida** | `pruebas-de-fuego/medida-w3-gobierno.py` | verbo a verbo, con dos personas, dos paquetes de dos equipos, un retículo y un conducto: el plano de datos **registraba y no decidía** (arriba, «Lo medido para W3.7 gobierno») |
| **①** la puerta del puesto | ORE `fef6d7b` | `rutas.rs::puerta_del_agente`, lista de permitidos (P4): un agente no escribe fuera de `/puestos`, `/v1`, `/documentos`, `/conceptos` y `confirmar`. **Decide el sujeto, no la cabecera.** `DELETE /documentos/Dataset` se lleva el puntero |
| **②** el conducto de la lectura | ORE `98fcd31` | `flow::carga_de` + `flow::fugas` (con la vía nueva: las columnas de la `Table` raíz) y `flow::lectura_desde_puesto`; `datos` niega con `OOS4002` y trae `clasificacion`; sin `contextSurface.workspace` se coteja con `materialization.payload`; los tres SDK lo dicen en la celda |
| **②b** la credencial de lectura | ORE `83f3bdd`, `372f395` | `prestar {modo: leer}`, `--prestar --leer`, `datos` con `credencial` (un secreto de DuckDB por raíz, con `scope`); la capa se baja **por su nombre**; `ore-puesto-<n>` pasa a `objectViewer` **con condición** sobre `ore/puesto/`; `malla/empujar-plantilla.py` |
| **③** la clasificación por el grafo | OOS `5d54854` + ORE `2215b35` | **`derivedFrom`** en el dataset escrito (spec §3.2/§5/§7, esquema, tres casos de conformance); `--commit` lo escribe de la procedencia (`Leyo`); `flow::carga_de` vía 3, recursiva; el índice saca `sale_de` del documento |
| **④** quién reescribe, y la rama | ORE `40cd975` + rubix-platform `a8e406c` | **403 y no 409** (no es un conflicto: no es suyo) en la credencial, el commit y el retirar, por `escrito_por`; `rama_del_puesto` + `Forja::asegurar_rama`; «Publish» en la fila de la sesión |
| **⑤** lo declarado, en el servidor | ORE `00f52af` | `POST`/`DELETE /puestos/{id}/transform`; `datos` sólo `inputs`, el catálogo sólo `output`; `GET /puestos` lo dice y el informe gana `declarado` |

**La misma medida, al final** (`medida-w3-gobierno.py`, 2026-09-22; las cinco secciones):

| | antes | ahora |
|---|---|---|
| **§1 leer** | `over("ventas.salida")` desde ana **y desde bob** devolvía la columna `high` entera; `datos` no traía clasificación | **`OOS4002`** en ana, bob, la View y `sql()`; `datos` 403 con `codigo`, y con el conducto abierto trae `clasificacion` y `credencial`. En el índice, la View y el Dataset pasan de `{}` a `high` |
| **§2 escribir** | bob anexaba, sobrescribía y retiraba lo de ana (200, 200, 200); desde una celda se reescribían `conduits.yaml` y el `owner` del paquete (200, firmado `agente:local`) | **403** los tres primeros (con `persona:ana`) y **403** todo lo de `/arbol`, `/ramas`, `/propuestas`, `/paquetes` desde un puesto. Lo de bob vive en `bob/puesto` |
| **§3 declarar** | bob redeclaraba la Entity de ana y **la desclasificaba en `main`** (el índice pasaba a `low`) | lo redeclarado queda **en su rama**: `main` sigue `high`, firmado por ana, y la rama dice `low` hasta que una propuesta la traiga |
| **§4 correr** | dentro del transform, `pedir()` a pelo era 200 y `PUT /arbol` 201 | **`['PermissionError', 403, 403, 'resumir', ['ventas.salida'], 3]`**: el servidor acota lo declarado, y `PUT /arbol` es 403 |
| **§5 el grafo** | lo escrito salía sin clasificar y una copia encima **compilaba** con el conducto `low` | `derivedFrom: [ventas.salida]` en el documento, `high` en el índice, `sale_de` sin ciclo, y la copia encima es **`OOS4002`** |
| **§6 en la fuente** | el conducto sólo en `autorizar_conducto` (hacía nacer, no decidía) | `flow` decide en tres sitios (el compilador, el índice, la lectura); `escrito_por` decide en tres más; la concesión sigue donde estaba (`mando.rs`: otra cosa) |

**Lo que salió al construir, y no estaba en la decisión:**

- **Lo que baja va en el documento, no en el puntero**: el compilador no lee estado, y la carga
  de un dataset tiene que verse con el árbol solo. De ahí `derivedFrom` en OOS, y no «leer la
  procedencia».
- **Es 403, no 409**: la regla 4 decía 409. Un conflicto se resuelve reintentando; esto no.
- **Decide el sujeto, no la cabecera**: quitar `x-ore-puesto` desde la celda dejaba el mismo
  testigo del agente, así que gatear por la cabecera era teatro.
- **Una condición IAM sobre el nombre no da `objects.list`**, ni con prefijo: por eso la capa
  se baja por su nombre (`requirements.lock` dice cuáles), y no listando.
- **Sin `contextSurface.workspace` se coteja con `materialization.payload`**: la copia ya salió
  por él al bucket del inquilino, y leerla desde un puesto del mismo inquilino no la lleva más
  lejos. Consecuencia buscada: demo y victor no cambian hasta que declaren un retículo.
- **Una rama nace de `main` y no lo sigue**: publicar y volver a abrir es cómo una sesión
  recoge lo nuevo. La medida necesitó un `publica()` para que ana y bob se vieran.
- **La CI vio lo que yo no**: `el-puesto.sh` se salta el caso 9 sin `javac` en el PATH, y el SDK
  de Java no compilaba. El caso 9 no es opcional: si no corre, no está probado.

**Lo que queda fuera, con dueño:** la procedencia sigue siendo **lo que el SDK vio** (quien lo
...
el `owner: team:…` sigue siendo documentación hasta que **ore-iam** diga quién es de qué equipo
—y entonces será él quien acepte la propuesta—; y los sobres heredados (`clave`, `ore/v1/`)
