...
| **3** | la medida viva | **hecho** 2026-09-21 (`medida-assets-catalog.py`, contra el ore-serve vivo): demo **frío 815 ms / caliente 102 ms**, 16 KB, 17 ítems, `version` en 17/17; victor **frío 1 362 ms / caliente 50 ms**, 76 KB, 58 ítems, `version` en 58/58. Frente a lo que el catálogo pedía: la medida hizo 14 llamadas / 7,4 s (demo) y 12 / 11,2 s (victor) por `/paquetes` + `/esquema` + `/copias` por paquete + `/datasets`; la consola, sólo con las bases elegidas, ~6 llamadas ≈ 3–5 s. Comprobado en victor: `dataset:standard_test.ai_insights` → `identidad: true`, `sale_de table:standard_test.public_ai_insights`, puntero `copiada · 4 filas`, `version 073d54f`; `table:foreign_test.public_ShopifyStore` → `vistaInducida: view:foreign_test.shopifyStore`. En demo el dataset dice su puntero en `error` con el motivo tal cual |
| **4** | la consola sobre el índice | **hecho** 2026-09-21 (rubix-platform `e03128b`, local): `GET /assets` por el proxy, la página del catálogo hace UNA llamada; `comoDatabaseDesdeAssets` pinta paquete › carpeta («default» la raíz) › ítems; Tables y Datasets en la lista principal, Views propias y Entity/Interface/Concept/Function/Action/TrainedModel en secciones por kind con icono propio en el árbol; `AssetDetail` para todo lo que no es Table: Overview (define, expone), Links (relaciones agrupadas, cada una se abre, rota se dice), Access (clasificación y conductos; la concesión de iam dicha como del plano de control), History (último commit), Copy en un Dataset. `comoDatabase`, `datasets.ts` y `DatasetDetail` fuera. `tsc` limpio; `next build` no (el `next dev` del usuario). Lo que queda para cuando se mida: la historia git entera y los snapshots bajo demanda (`/arbol/historia`, `/datasets/{ns}/{n}`), y crear un schema desde la consola (`README.md` en la carpeta por `PUT /arbol`) |
| **5** | `Function`/`Action` por `/documentos` | **hecho** 2026-09-21: dos filas más en `KINDS` (`functions/`, `actions/`, sin exigencias propias: lo que exigen ya lo exige el compilador). `los-documentos.sh` caso 18b: la función de sólo lectura entra (201), se lista por su kind en `functions/`, se lee entera y se retira; una Action que escribe una propiedad sin integridad es 422 con OOS7005 y no queda — la puerta compila la Action, no sólo la guarda. El caso 11 («un kind que no está en la tabla») pasa a probar con `Ruleset` |
