...
| antes | después | nota |
|---|---|---|
| `View v` con `materialized` y `from: {table: t}`, sin `fields`/`where`, **`v` se llama como `t`** (lo que `discover` emitía) | **sólo** `Dataset v { from: {table: t} }`; la View desaparece | la copia identidad: la vista era la tabla con otro nombre |
| `View v` con `materialized` y plan (`fields`/`where`/agregado) | `View v` **sin** `materialized` + `Dataset v { from: {view: v} }` | la pregunta sigue; sus filas guardadas tienen documento. Si nadie lee `v` como vista, quien migra lo puede fundir después |
...
| `copias/<p>_<v>.json` | `datasets/<p>_<v>.json` | `git mv`; el contenido no cambia |
...
| **4** | migrar `demo` y `victor`; CI verde; el aprovisionador converge | `GET /datasets` igual antes/después; el catálogo lista lo mismo; un `write()` desde un puesto deja un Dataset escrito |
| **5** | la consola | `next build`; Create › Dataset abre el workspace con el documento; la sección Datasets lista `GET /datasets` del paquete |
