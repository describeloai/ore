...
pub fn shape_rules() -> Vec<ShapeRule> {
    vec![
        ...
        // `field` solo significa algo con su testigo, y `key` con `upsert`
        // hace falta. Lo que ya NO se comprueba es que `key` sobre: hasta
        // v1alpha8 solo se admitia con `upsert` y era demasiado estrecho,
        // porque LA CLAVE LA LEEN TRES y solo una es el upsert —el otro es
        // fundir un incremento con la copia, con cualquier modo, y el tercero
        // es saber que fila toca un edit—. La regla vieja dejaba a una tabla
        // `retract` con vista materializada sin poder declarar clave, o sea
        // sin fusion incremental. La clave es la identidad de la fila y eso no
        // depende de por que se pregunte.
        ShapeRule {
            kind: Kind::Table,
            path: &["spec", "changes"],
            check: |n| {
